...
def getdata():
    ...
    r = (0.25+t/10)
    X = (numpy.array([r*numpy.sin(2*t)+1,r*numpy.sin(1.1*t-0.5)])).T
    Y = (numpy.array([r*numpy.sin(t),   -r*numpy.cos(t)+1])).T
    return X,Y

def plotdata(X,Y):
    f = plt.figure(figsize=(8,4))
    p1 = f.add_subplot(1,2,1); p1.set_xlim(-3,3); p1.set_ylim(-3,3)
    p1.scatter(X[:,0],X[:,1],c=numpy.arange(len(X)))
    p2 = f.add_subplot(1,2,2); p2.set_xlim(-3,3); p2.set_ylim(-3,3)
    p2.scatter(Y[:,0],Y[:,1],c=numpy.arange(len(Y)))
    return p1,p2
...
def getHDdata():
    ...
    return X.reshape(-1,10000),Y.reshape(-1,10000)
